    ...
    setup() {
        ...
        while(curpathlength < pathlength) {
            ...
            if(Math.random() < 0.5 || first) {
                ...
                if(tile1 !== false) path1.push(tile1);
                else {
                    while(true) {
            ...
            if(Math.random() >= 0.5 || first) {
                ...
                if(tile2 !== false) path2.push(tile2);
                else {
                    while(true) {
                        let checkcoord2 = Math.floor(Math.random()*gridheight)+','+Math.floor(Math.random()*gridwidth);
                        if(!path1.includes(checkcoord2) && !path2.includes(checkcoord2) && getNextTile(checkcoord2) !== false && checkcoord2 !== checkcoord) {
                ...
                if(pathIndex !== -1) {
                    if(start == coord) {
                        ...
                        if(nextDir == 'portal' || prevDir == 'portal') {
                            ...
                            type = 'I';
                            rotation = dirs.indexOf(nextDir) % 2;
                        } else {
                            type = 'L';
                            let d1 = dirs.indexOf(prevDir), d2 = dirs.indexOf(nextDir);
                            if((d1==0 && d2==3) || (d1==3 && d2==0)) rotation = 3;
                            else rotation = Math.min(d1, d2);
                        }
                    }
        ...
        function getColour(tile, rowIndex, colIndex) {
            if(tile[0] === 'S') {
                tile[3] = 'A';
                if(!paths['A'].includes(rowIndex+','+colIndex)) paths['A'].push(rowIndex+','+colIndex);
                return colours['A'];
            }
            
            const currentCoord = rowIndex+','+colIndex;
            const checkConnect = (r, c, fromDir) => {
                if (r < 0 || r >= gridheight || c < 0 || c >= gridwidth) return false;
                const adj = grid.value[r][c];
                if (adj[3] === '') return false;
                
                const adjRot = adj[1];
                const adjType = adj[0];
                let adjOpenings = [];
                if(adjType == 'I') adjOpenings = [dirs[adjRot], dirs[(adjRot+2)%4]];
                else if(adjType == 'L') adjOpenings = [dirs[adjRot], dirs[(adjRot+1)%4]];
                else if(adjType == 'S' || adjType == 'E' || adjType == 'P') adjOpenings = [dirs[adjRot]];
                
                return adjOpenings.includes(fromDir);
            };

            let openings = [];
            const rot = tile[1];
            if(tile[0] == 'I') openings = [dirs[rot], dirs[(rot+2)%4]];
            else if(tile[0] == 'L') openings = [dirs[rot], dirs[(rot+1)%4]];
            else if(tile[0] == 'E' || tile[0] == 'P') openings = [dirs[rot]];

            let connected = false;
            if (openings.includes('up') && checkConnect(rowIndex-1, colIndex, 'down')) connected = true;
            if (openings.includes('down') && checkConnect(rowIndex+1, colIndex, 'up')) connected = true;
            if (openings.includes('left') && checkConnect(rowIndex, colIndex-1, 'right')) connected = true;
            if (openings.includes('right') && checkConnect(rowIndex, colIndex+1, 'left')) connected = true;

            if (tile[0] === 'P' && !connected) {
                for(let i=0; i<gridheight; i++) {
                    for(let j=0; j<gridwidth; j++) {
                        const other = grid.value[i][j];
                        if(other[0] === 'P' && other[4] === tile[4] && other[3] !== '' && (i !== rowIndex || j !== colIndex)) {
                            connected = true;
                        }
                    }
                }
        ...
        <div v-for="(row, rowIndex) in grid" :key="rowIndex">
            <div style="display: inline-block" v-for="(tile, colIndex) in row" :key="colIndex">
                <div :class="['tile', getMovableClass(tile[2])]" :style="'rotate: '+tile[1]*90+'deg;'" @click="rotate(tile, rowIndex, colIndex)">
                    ...
                    <div v-if="tile[0] == 'P'" :class="['portalsymbol', 'grandiflora-one-regular', { active: tile[3] !== '' }]" :style="'rotate: '+tile[1]*(-90)+'deg;'">
                        {{shapes[tile[4]]}}
                    </div>
                </div>
            </div>
