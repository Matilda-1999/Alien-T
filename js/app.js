...
export default {
    setup() {
        ...
        // 포털(P) 및 시작/종료: 연결구가 향하는 방향 0:하, 1:좌, 2:상, 3:우

        // 패턴 A: 시작(0,0) -> 종료(3,11) [모든 구간 L자 3개 이상, 6칸 이상]
        const patternA = [
            // 구간 1: 시작(0,0) -> 포털0(2,3) [8칸, L자 4개]
            [0,0,'S',3], [0,1,'L',1], [1,1,'L',3], [1,2,'I',1], [0,2,'L',1], [0,3,'L',2], [1,3,'I',0], [2,3,'P',2,0],
            // 구간 2: 포털0(3,8) -> 포털1(6,10) [7칸, L자 4개]
            [3,8,'P',0,0], [4,8,'L',0], [4,9,'L',2], [5,9,'L',0], [5,10,'L',2], [6,10,'P',2,1],
            // 구간 3: 포털1(4,2) -> 포털2(6,5) [6칸, L자 4개]
            [4,2,'P',3,1], [4,3,'L',1], [5,3,'L',3], [5,4,'L',1], [6,4,'L',3], [6,5,'P',1,2],
            // 구간 4: 포털2(7,0) -> 종료(3,11) [14칸, L자 6개]
            [7,0,'P',3,2], [7,1,'L',0], [6,1,'L',2], [6,2,'L',0], [5,2,'L',2], [5,3,'L',0], [4,3,'L',2], [4,4,'I',1], [4,5,'L',0], [3,5,'L',2], [3,6,'I',1], [3,7,'I',1], [3,8,'I',1], [3,11,'E',1]
        ];

        // 패턴 B: 시작(0,0) -> 종료(5,11) [모든 구간 L자 3개 이상, 6칸 이상]
        const patternB = [
            // 구간 1: 시작(0,0) -> 포털0(3,2) [6칸, L자 3개]
            [0,0,'S',3], [0,1,'L',1], [1,1,'I',0], [2,1,'L',0], [2,2,'L',2], [3,2,'P',2,0],
            // 구간 2: 포털0(4,11) -> 포털1(3,8) [7칸, L자 3개]
            [4,11,'P',1,0], [4,10,'L',3], [3,10,'I',1], [3,9,'L',2], [4,9,'I',1], [4,8,'L',3], [3,8,'P',0,1],
            // 구간 3: 포털1(1,0) -> 포털2(4,1) [7칸, L자 4개]
            [1,0,'P',3,1], [1,1,'L',1], [2,1,'L',0], [2,2,'L',2], [3,2,'I',1], [3,1,'L',2], [4,1,'P',2,2],
            // 구간 4: 포털2(1,5) -> 종료(5,11) [12칸, L자 4개]
            [1,5,'P',0,2], [2,5,'I',0], [3,5,'L',1], [3,6,'I',1], [3,7,'L',1], [4,7,'L',2], [4,6,'L',0], [5,6,'I',1], [5,7,'I',1], [5,8,'I',1], [5,9,'I',1], [5,11,'E',1]
        ];

        const selectedPattern = Math.random() < 0.5 ? patternA : patternB;
        const grid = ref([]);
        const redStartSet = ref(new Set()); // 시작점으로부터 흐르는 전류
        const redEndSet = ref(new Set());   // 끝점으로부터 흐르는 전류
        ...
        const getOpenings = (r, c) => {
            ...
            if (tile[0] === 'L') return [(rot + 2) % 4, (rot + 3) % 4]; // [보정 완료]
            return [];
        };
        ...
        const updatePower = () => {
            ...
            redStartSet.value = runFloodFill(startNode[0] + ',' + startNode[1]);
            redEndSet.value = runFloodFill(endNode[0] + ',' + endNode[1]);
        };
        ...
        const isGameWon = computed(() => {
            const endNode = selectedPattern.find(p => p[2] === 'E');
            return redStartSet.value.has(endNode[0] + ',' + endNode[1]);
        });
        ...
        <div v-for="(row, rowIndex) in grid" :key="rowIndex">
            <div style="display: inline-block" v-for="(tile, colIndex) in row" :key="colIndex">
                <div :class="['tile', getMovableClass(tile[2])]" 
                    ...
                    <div v-if="tile[0] == 'P'" class="portalsymbol active" :style="{ color: '#1a1c16', transform: 'rotate(' + (isCorrectPath(rowIndex, colIndex) ? tile[5]*-90 : tile[1]*-90) + 'deg)' }">
                        {{tile[4] + 1}}
                    </div>
                </div>
            </div>
