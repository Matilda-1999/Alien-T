...
export default {
    setup() {
        ...
        const patternB = [[0,0,'S',3], [0,1,'L',3], [0,5,'P',3,0], [0,6,'L',3], [1,1,'I',0], [1,2,'L',2], [1,3,'I',1], [1,4,'I',1], [1,5,'I',1], [1,6,'L',0], [2,1,'L',1], [2,2,'L',0], [2,3,'P',3,1], [3,3,'I',0], [3,5,'L',2], [3,6,'I',1], [3,7,'L',3], [3,8,'L',2], [3,9,'I',1], [3,10,'L',3], [4,3,'L',1], [4,4,'L',3], [4,5,'P',3,2], [4,7,'I',0], [4,8,'I',0], [4,9,'P',3,2], [4,10,'I',0], [5,3,'L',2], [5,4,'L',0], [5,6,'L',2], [5,7,'L',0], [5,8,'I',0], [5,9,'I',0], [5,10,'I',0], [5,11,'E',2], [6,3,'I',0], [6,5,'L',2], [6,6,'L',0], [6,7,'P',3,1], [6,8,'L',1], [6,9,'L',0], [6,10,'I',0], [6,11,'I',0], [7,0,'P',3,0], [7,1,'I',1], [7,2,'I',1], [7,3,'L',0], [7,5,'L',1], [7,6,'I',1], [7,7,'L',0], [7,10,'L',1], [7,11,'L',0]];
        ...
        const getOpenings = (tile, r, c) => {
            // 정답 모드일 경우 정답 각도를, 아닐 경우 현재 각도를 사용하여 물리적 연결 판정
        ...
        for (let i = 0; i < gridheight; i++) {
            ...
            for (let j = 0; j < gridwidth; j++) {
                ...
                if (p) {
                    ...
                    rotation = movable === 0 ? ansRot : (ansRot + Math.floor(Math.random() * 3) + 1) % 4;
                } else {
        ...
        <div v-for="(row, r) in grid">
            <div style="display: inline-block" v-for="(tile, c) in row">
                <div :class="['tile', tile[2] === 0 ? 'tile-immovable' : 'tile-movable']" 
                     :style="{ 
                        /* [보정] L자 타일은 CSS 기본 모양에 맞춰 180도(+2) 추가 회전 */
                        transform: 'rotate(' + ((isCorrectPath(r, c) ? tile[5] : tile[1]) + (tile[0] === 'L' ? 2 : 0)) * 90 + 'deg)', 
                    ...
                    <div class="circle-node" :style="{ backgroundColor: getColour(r, c) || '#d4af37', transform: 'rotate(' + ((isCorrectPath(r, c) ? tile[5] : tile[1]) + (tile[0] === 'L' ? 2 : 0)) * -90 + 'deg)' }"></div>
                    <div v-if="tile[0] == 'P'" class="portalsymbol active" :style="{ transform: 'rotate(' + ((isCorrectPath(r, c) ? tile[5] : tile[1]) + (tile[0] === 'L' ? 2 : 0)) * -90 + 'deg)' }">{{tile[4] + 1}}</div>
                </div>
            </div>
        </div>
    </div>`
