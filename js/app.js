...
export default {
	setup() {
		...
		// 무한 로딩 방지: while(true) 제거 및 safetyCounter 도입
		let first = true;
		let totalAttempts = 0;
		while(curpathlength < pathlength && totalAttempts < 1000) {
			totalAttempts++;
			if(Math.random() < 0.5 || first) {
				const t1 = getNextTile(path1[path1.length-1], path1, path2);
				if(t1) { path1.push(t1); curpathlength++; }
				else {
					// 포털 찾기 (무한루프 방지를 위해 최대 100번만 시도)
					for(let i=0; i<100; i++) {
		...
		return { 
			...
			isCorrectPath: (r, c) => showAnswer && fullpath.includes(r + ',' + c) // 정답 표시 기능
		};
	},
	template: `
