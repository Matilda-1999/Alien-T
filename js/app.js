...
export default {
	setup() {
		...
		// 'topsecret' 파라미터가 true일 때 정답 경로 표시
		...
		const fullpath = path1.concat(path2.toReversed()); // 전체 정답 경로
		...
		{
			if(type == "X") return "empty-node";
			else if(type == "I") return "i-node";
			else if(type == "L") return alt ? "lh-node" : "lv-node";
			else if(type == "S") return "start-node";
			else if(type == "E") return "end-node";
			else if(type == "P") return "portal-node";
			else if(type == "D") return "deadend-node";
		}
		
		function getMovableClass(a)
		{
			return a == 0 ? "tile-immovable" : "tile-movable";
		}
		
		function getColour(tile, rowIndex, colIndex)
		{
			if(tile[0] === 'S') return colours[tile[3]];
			else
			{
				const dirs = ['up', 'right', 'down', 'left'];
				let openings = [];
				if(tile[0] == 'I') openings = [dirs[tile[1]], dirs[(tile[1]+2)%4]];
				else if(tile[0] == 'L') openings = [dirs[tile[1]], dirs[(tile[1]+1)%4]];
				else if(tile[0] == 'E') openings = [dirs[(tile[1]+2)%4]];
				else if(tile[0] == 'P')
				{
						...
						for(let j = 0; j < grid.value[i].length; j++)
						{
							...
							if(adjTile[0] == 'P' && adjTile[4] == tile[4] && adjTile[3] !== '')
							{
								...
								if(adjTileIndex !== -1 && (tileIndex === -1 || tileIndex > adjTileIndex))
								{
									...
									if(paths[colour].indexOf(coord) === -1) paths[colour].push(coord);
				...
				if(grid.value[rowIndex][colIndex][3] !== '')
				{
					...
					if(index !== -1)
					{
						...
						for(let i = 0; i < removed.length; i++)
						{
							...
							if(grid.value[coords[0]][coords[1]][3] == colour) grid.value[coords[0]][coords[1]][3] = '';
						}
					}
				}
		...
		{
			if(tile[2]) grid.value[rowIndex][colIndex][1] = (grid.value[rowIndex][colIndex][1] + 1)%4;
		}
		...
		function checkWinStatus()
		{
			...
			for(let i = 0; i < grid.value.length; i++)
			{
				for(let j = 0; j < grid.value[i].length; j++)
				{
					if(grid.value[i][j][0] == 'E' && grid.value[i][j][3] !== grid.value[i][j][4]) winCondition = false;
				}
			}
			return winCondition;
		}
		...
		// 정답 경로 시각적 확인용 함수
		function isCorrectPath(rowIndex, colIndex) {
			if (!showAnswer) return false;
			return fullpath.includes(rowIndex + ',' + colIndex);
		}
		
		return { grid, getNodeClass, getMovableClass, shapes, getColour, rotate, colours, checkWinStatus, isCorrectPath };
	},
	template: `<div>
		...
		<div v-for="(row, rowIndex) in grid">
			<div style="display: inline-block" v-for="(tile, colIndex) in row">
				<div :class="'tile ' + getMovableClass(tile[2])" 
					 ...
					 v-if="tile.length != 0" @click="rotate(tile, rowIndex, colIndex)">
					
					<div :style="{ 
						backgroundColor: (tile[0] === 'S' || tile[0] === 'E') ? '#FF4500' : getColour(tile, rowIndex, colIndex),
						boxShadow: (tile[0] === 'S' || tile[0] === 'E') ? '0 0 15px #FF4500' : 'none'
					}" :class="getNodeClass(tile[0])"></div>
					...
					<div v-if="tile[0] != 'X'" :class="(tile[0] === 'P' ? 'bigcircle-node' : 'circle-node')" 
						 :style="'background-color: '+(tile[0] == 'E' ? '#FF4500' : getColour(tile, rowIndex, colIndex))+'; rotate: '+tile[1]*(-90)+'deg;'"></div>
					
					<div v-if="tile[0] == 'P'" :class="'portalsymbol grandiflora-one-regular' + (getColour(tile, rowIndex, colIndex) !== '' ? ' active' : '')" :style="'rotate: '+tile[1]*(-90)+'deg;'">
						{{shapes[tile[4]]}}
					</div>
				</div>
			</div>
